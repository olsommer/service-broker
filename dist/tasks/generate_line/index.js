...
async function generate(record) {
    ...
    try {
        ...
        // Get summary data
        // --------------------------------------
        const { data: sumData, error: sumErr } = await supabase_1.supa
            .from("summaries")
            .select("*")
            .eq("lead_job_id", id)
    ...
    `;
        ...
        if (error)
            throw error;
        await (0, next_1.setNextState)(id, "FLAG_TO_FINISH");
    }
    catch (error) {
        console.error(error);
        await (0, log_1.log)("ERROR", error.message, id, "generate");
    }
}
